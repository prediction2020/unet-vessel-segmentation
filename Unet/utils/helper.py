...
def load_patches(data_dir, patient, patch_size):
    ...
    """
    patch_path = data_dir + 'patch' + str(patch_size) + '/' + str(patient) + '_' + str(patch_size)
...
def read_tuned_params_from_csv(tuned_params_file):
    """
    Reads tuned hyperparameters used in training from a csv file.
    :param tuned_params_file: csv file with tuned params.
    :return: tuple with lists of hyperparameters.
    """
    patch_size_list = []
    num_epochs_list = []
    batch_size_list = []
